...
var layoutService = withObservable.call({
    ...
    updateViewportSize: function () {
        this.viewportSize.x = Math.max(
            document.documentElement.clientWidth,
            window.innerWidth || 0
        );
        this.viewportSize.y = Math.max(
...
window.addEventListener(
    "resize",
    debounceUpdateListener({
        timeout: 500,
        debounce: 250,

        start: function () {
            layoutService.notify(
                layoutService.EVENT_START,
                layoutService.updateViewportSize()
            );
        },
        update: function () {
            layoutService.notify(
                layoutService.EVENT_UPDATE,
                layoutService.updateViewportSize()
            );
        },
        end: function () {
            layoutService.notify(
                layoutService.EVENT_END,
                layoutService.updateViewportSize()
            );
        },
    })
